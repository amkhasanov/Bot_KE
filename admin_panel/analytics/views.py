...
def show_admin_custom_page(request):
    ...
    print(clicks_button_per_month, 'month')
    ...
    count = []
    print(count_clicks_per_day)
    print(count_clicks_per_week)
    print(count_clicks_per_month)
    print(count_clicks_per_all_time)
    clicks_count = {}
    for button_title in count_clicks_per_all_time.keys():
        try:
            count_per_day = count_clicks_per_day[button_title]
        except KeyError:
            count_per_day = 0
        try:
            count_per_week = count_clicks_per_week[button_title]
        except KeyError:
            count_per_week = 0
        try:
            count_per_month = count_clicks_per_month[button_title]
        except KeyError:
            count_per_month = 0
        try:
            count_per_all_time = count_clicks_per_all_time [button_title]
        except KeyError:
            count_per_all_time = 0
